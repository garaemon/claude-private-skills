...
Usage:
    gather_diff.py                       # whole branch (default)
    gather_diff.py --last 1              # only the most recent commit
    gather_diff.py --last 3              # the last three commits
    gather_diff.py --commit abc123       # that one commit
    gather_diff.py --range abc123..def456
    gather_diff.py --base origin/develop # whole branch, against another base
    gather_diff.py --patch               # any of the above, plus the full diff
    gather_diff.py --patch-for cmd/x.go  # plus one file's diff
...
def build_commit_range(revision):
...
def resolve_explicit_range(args):
