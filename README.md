...
- [check-updates](.claude/skills/check-updates/SKILL.md) — Scan every skill under `.claude/skills/<name>/` for pinned third-party dependencies (npm, PyPI, GitHub releases via `go install … @${VAR}`) and report which ones have newer upstream releases. Read-only — prints a markdown or JSON report, never modifies files or rebuilds images.
